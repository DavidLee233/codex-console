...
class IMAPNewProvider(OutlookProvider):
    ...
    IMAP_PORT = 993
    MAILBOX_CANDIDATES = IMAPOldProvider.MAILBOX_CANDIDATES
    ...
    def inject_test_code_email(self, code: str, mailbox: str = "INBOX") -> bool:
        ...
        try:
            ...
            if ok:
                logger.info(f"[{self.account.email}] IMAP_NEW APPEND 测试邮件成功, code={code}")
            else:
                logger.warning(f"[{self.account.email}] IMAP_NEW APPEND 测试邮件失败, status={status}")
            return ok
        except Exception as e:
            logger.warning(f"[{self.account.email}] IMAP_NEW APPEND 测试邮件异常: {e}")
            return False
